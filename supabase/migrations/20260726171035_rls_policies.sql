...
-- 사용자 세션 없이 도는 유일한 경로이므로 전 권한을 부여한다 — 였다.
--
--   20260729155440 이 아래 두 grant 를 전부 회수한다. 부여의 근거였던 "시세
--   배치"가 **저장소에 존재하지 않으므로** 필요 권한이 0이었고, 그 사이
--   service_role 은 12개 테이블에 8종 권한을 갖고 있었다. 이 롤은
--   rolbypassrls 이므로 정책이 2차 필터로 남지 않는다 — 권한 층이 전부다.
--   배치가 생기는 날(P5a 컷 4) 필요분만 명시 부여한다. AQ-11.
--
