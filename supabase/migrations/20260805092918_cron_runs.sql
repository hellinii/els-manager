...
--
-- ---------------------------------------------------------------------------
-- ★★ actor_id 는 «위조 불가» 다 — 그러나 운영에서 ② 를 «닫지 못한다» (P5a 컷 5).
--
--   AQ-57 이 ⓐ(GoTrue 사용자 계정)로 닫혔으므로 배치가 세션을 갖는다. 그래서 아래
--   INSERT 정책이 actor_id = auth.uid() 를 강제할 수 있고, 그것이 귀속을 «위조 불가»
--   로 만든다. caller 는 요청이 스스로 주장하는 값이므로 편의상의 분류일 뿐
--   판별자가 아니다 — 그 비대칭을 여기 적어 둔다.
--
--   ★ 그런데 운영은 ⓐ′ 다 — 배치가 «민서 자신의 계정» 으로 로그인한다.
--     그러면 actor_id 가 모든 행에서 같아 Cron·SCR-302 버튼·진단 curl 을 «가르지
--     못한다». 위조 불가성이 사라진 것이 아니라 «가리키는 것» 이 바뀌었다 —
--     「누가」는 여전히 참인데 그 답이 언제나 한 사람이다. 근거 ② 는 닫히지 «않았고»
--     잔여는 DOC-013 AQ-69 다. 근거 ①·③ 은 온전하므로 이 테이블은 정당하다.
--
--     주석만 정정한다 — DDL 은 한 줄도 바뀌지 않았고 이 파일은 아직 운영에 적용되지
--     않았다(적용 전 정정이므로 이력이 갈리지 않는다).
--
