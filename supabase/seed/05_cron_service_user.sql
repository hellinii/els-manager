-- ============================================================================
-- 배치 계정 (로컬·e2e 전용) — P5a 컷 3 (DOC-010 AQ-57 = ⓐ · DOC-013 §7)
--
-- ★★★★ 운영은 이 계정을 «쓰지 않는다» — ⓐ′다 (P5a 컷 5, DOC-013 §10.3).
--
--   민서가 «자기 계정»을 배치 계정으로 재사용하기로 정했다. 근거는 그 계정의 비밀번호가
--   기계 생성이고 회전 계획이 없다는 것이며, 그것이 전용 계정을 권한 가장 실질적인
--   논거(사람 비밀번호는 회전하고 그 실패가 조용하다)를 무력화한다.
--
--   **이 파일은 그대로 남는다. 어긋남이 의도다:**
--     · 정체성은 «환경변수의 값»이지 코드 경로가 아니다 — e2e가 그 경로를 실 HTTP로
--       태우므로 검증 손실이 없다
--     · 반대로 로컬도 사람 계정을 쓰면 e2e의 «사용자별 집계» 단언이 배치가 만든 행에
--       흔들린다(AQ-34가 전용 사용자를 만든 이유와 같은 축)
--
--   대가는 `cron_runs` 근거 ② 하나이며 AQ-69로 등재했다 — 운영에서 `actor_id`가 모든
--   행에서 같아 「배치가 썼는가」를 위조 불가로 말하는 열이 없다.
--
...
--
-- ★★ `cron_runs.actor_id`는 «위조 불가»다 — 그러나 운영에서 «판별하지» 못한다.
--
--   정책이 `with check (actor_id = (select auth.uid()))`이므로 다른 값을 적으려면
--   42501이다. 즉 그 열은 「누가 썼다고 주장하는가」가 아니라 **「누가 썼는가」**다.
--   그 성질은 ⓐ·ⓐ′ 양쪽에서 참이다(`service_role`은 rolbypassrls 라 얻지 못한다).
--
--   ~~ⓐ의 네 번째 논거이며 AQ-51·AQ-57의 상승효과다~~ → **운영에서는 성립하지 않는다.**
--   ⓐ′에서 배치와 사람이 같은 계정이라 `actor_id`가 모든 행에서 같고, 「배치가
--   썼는가」에 답하는 것은 `caller`뿐인데 그것은 요청이 스스로 주장하는 값이다.
--   **위조 불가성이 사라진 것이 아니라 가리키는 것이 바뀌었다** — AQ-69.
--
...
--
--   이 파일은 **로컬 스택 전용**이다(`db:reset`이 적용한다). 운영에서는 계정을
--   새로 만들지 «않고» 민서 자신의 계정을 쓰며(ⓐ′), 그 비밀번호를 Vercel 환경변수
