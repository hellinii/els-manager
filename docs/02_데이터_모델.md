...
**제약:** `UNIQUE(els_id)` — 상품은 1회만 상환된다, `CHECK(redemption_type <> 'MATURITY_LOSS' OR taxable_income = 0)` — I-08
...
I-01 ~ I-06, I-08, I-10은 DB 제약으로 강제한다(DOC-010 §8 K-04). I-07은 교차 행 규칙이라 단일 행 제약으로 표현할 수 없으므로 애플리케이션이 담당하며 DOC-011 §6의 V-02·V-03에 대응한다. I-09는 `redemptions`에 소유자 컬럼을 두지 않는 것으로 구조적으로 성립한다 — 소유자는 `els_products.owner_id` 하나뿐이다.
