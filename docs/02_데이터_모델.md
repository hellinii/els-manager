...
| I-07 | 상품은 최소 1개의 기초자산과 1개의 차수를 가진다 | 애플리케이션 레벨 검증 — **등록·수정·삭제 전 경로** (DOC-011 V-02·V-03, §5.3) |
...
I-01 ~ I-06, I-08, I-10 ~ I-16은 DB 제약으로 강제한다(DOC-010 §8 K-04). I-07은 교차 행 규칙이라 단일 행 제약으로 표현할 수 없으므로 애플리케이션이 담당하며 DOC-011 §6의 V-02·V-03에 대응한다 — **생성 시점만이 아니라 값을 줄이는 모든 경로에 적용된다.** I-09는 `redemptions`에 소유자 컬럼을 두지 않는 것으로 구조적으로 성립한다.
...
| DQ-05 | `total_rounds`를 유지할지 `count(redemption_schedules)`로 대체할지 | 미결 | **M-02 위반 상태.** 유도 가능한 값을 저장하면서 일정 행 수와의 정합성이 어디에도 없다 — 일정 0건인 상품에 `total_rounds = 99`가 저장된다. §4.6은 만기일의 정본을 일정 마지막 차수로 정했으나 `total_rounds`의 정본은 정하지 않았고, DOC-007 §4.1의 `n`과 §7.2의 적용 차수가 어느 쪽을 따르는지도 미정이다. **P3에서 결정** |
| DQ-06 | 값 범위 제약 도입 여부 | 미결 | `principal > 0`, `total_rounds >= 1`, `evaluation_period_months >= 1`, `barrier > 0`, `gross_amount >= 0`, `redemption_date >= issue_date`가 전부 무제약이다 — 음수 원금·0 차수·음수 배리어·발행일 이전 상환일이 모두 저장된다. 단일 행 불변식이라 `CHECK`로 표현 가능하며, DOC-011 V-01·V-07·V-08·V-10이 계약 계층에 이미 있으므로 I-08과 같은 2층 구조로 승격할지의 문제다 |
