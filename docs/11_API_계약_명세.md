...
| AQ-30 | 생성 타입의 쓰기 방향이 Q-08·W-04와 어긋난다 | 미결 (P3b가 첫 방어를 둔다) | **실측 (P3b 2단계).** `supabase gen types`의 `Insert`·`Update`는 금액·비율 열을 `number`로 선언한다 — `asset_prices.price`, `els_underlyings.base_price`, `tax_constants.value` 등. 즉 **타입을 그대로 따르는 쓰기 코드가 곧 절대 규칙 #2 위반**이고, 읽기 방향에서 `::text` + `defineColumns`로 만든 방어가 쓰기에는 하나도 없다. PostgREST는 `numeric` 열에 문자열을 받으므로 어긋나는 것은 생성 타입이며 값을 문자열로 보내는 것이 맞다. P3b는 열 사양에서 파생한 `InsertPayload<T>`로 캐스팅을 한 곳에 모아 **`number`를 실으면 타입 오류가 되게** 한다. **남는 문제:** ① `defineColumns`가 select에만 걸리듯 이 타입도 그것을 경유하는 쓰기에만 걸린다(AQ-19의 우회로와 같은 형태). ② `db:types` 재생성이 열 타입을 바꾸면 파생 타입이 조용히 따라간다 — 드리프트 대조는 수동 스위트에만 있다. ③ 테스트가 픽스처를 `pg`로 직접 넣으므로 그 경로는 이 타입을 지나지 않는다 |
