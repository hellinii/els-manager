...
| 작성일 | 2026-07-26 |
| 작성자 | 민서 |
| 선행 문서 | DOC-002 데이터 모델 v0.3, DOC-007 계산 로직 명세 v0.3, DOC-008 화면 목록, DOC-010 아키텍처 v0.2 |
| 상태 | 검토 중 |
...
**제약:** **상환 완료 상품은 삭제할 수 없다** → `CONFLICT`. `redemptions.els_id`의 FK가 `RESTRICT`이므로 DB가 거부한다(DOC-002 §8.1, I-01)
