...
> **I-07은 생성 시점 규칙이 아니다.** DOC-002 v0.4가 I-07의 적용 범위를 모든 변경 경로로 넓혔다. `createProduct`만 검증하면 등록 직후에 하위 행을 지워 0건에 도달할 수 있고, 그 상태에서 DOC-007 §3.1이 워스트오브 산출을 **거부**하므로 상품 상세(SCR-202)가 렌더링되지 않는다. `deleteUnderlying` 같은 계약을 만들지 않는 것 자체가 이 방어의 형태다 — 하위 행은 항상 상품 단위로 통째로 교체된다.
>
...
| V-03 | `schedules` (`createProduct`·`updateProduct`) | 길이 = `totalRounds` (I-07) |
...
| AQ-17 | 순수 모듈의 예외를 변경 계약에서 어떻게 변환할지 | 미결 | DOC-007 v0.4 §3.1이 기초자산 0건을 거부하도록 바뀌면서 워스트오브 산출이 예외를 던진다. **조회 계약은 §3.1이 이미 "시스템 오류는 예외로 전파하여 에러 경계에서 처리"로 규정했으므로 문제가 없다.** 그러나 변경 계약은 예외를 던지지 않아야 하므로(§3.1 `ActionResult`), DOC-010 §6.3의 "`lib/domain` 재계산 검증"이 순수 모듈의 예외를 어떤 `ErrorCode`로 변환할지 정해야 한다 — `INTERNAL`인지 `CONFLICT`인지가 화면 처리를 갈라놓는다(§3.2). 0건 상태는 계약을 경유하면 도달 불가하지만 DB에서는 여전히 도달 가능하다(DOC-010 AQ-14). **P3** |
