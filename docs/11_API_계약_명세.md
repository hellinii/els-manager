...
| AQ-26 | 필터 문법 격리의 검증 범위 | 미결 | §4.9 `escapeLikePattern`은 `\`·`%`·`_`·`*`·`,`·`.`·`(`·`)`·`:`·`"`를 이스케이프하는데 테스트는 `%`·`_`·`,` 셋만 본다. 특히 **`\`는 순서 의존적**이다(먼저 처리하지 않으면 자기가 넣은 백슬래시를 다시 이스케이프한다). 순서가 뒤집혀도 조용하며, 나머지 문자는 PostgREST 필터 문법을 쪼갤 수 있다. P3b의 입력 검증(AQ-10)과 함께 다룬다 |
| AQ-27 | 기준일 → 귀속연도 파생 지점의 이원화 | 미결 | Q-02·Q-03이 기준일을 한 값으로 묶었는데, 그 기준일에서 **귀속연도를 꺼내는 지점은 둘**이다 — `today.ts`의 `currentYear()`와 `tax.ts`가 직접 하는 문자열 자르기. 현재는 컨텍스트 조립이 형식을 이미 보장해 값이 같지만, 파생 규칙이 바뀌면 한쪽만 바뀐다. Q-02가 "요청당 한 번"을 구조로 만든 것과 같은 이유로 파생 지점도 하나여야 한다. P3b에서 정리 |
