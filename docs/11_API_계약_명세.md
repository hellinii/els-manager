...
| 작성일 | 2026-07-26 |
| 작성자 | 민서 |
| 선행 문서 | DOC-002 데이터 모델 v0.5, DOC-007 계산 로직 명세 v0.5, DOC-008 화면 목록, DOC-010 아키텍처 v0.6 |
| 상태 | 검토 중 |
...
| `22001`(길이) · `22003`(자릿수) · `22P02`(형식) · `23502`(널) | `VALIDATION_FAILED` | 값의 형태가 원인이다. **필드를 특정할 수 없는 경우가 있다** — V-19·V-20이 그 앞에서 잡는 이유다 |
...
> **제약 이름을 어디서 얻는지는 실측으로 정한다.** `PostgrestError`는 `{message, details, hint, code}`뿐이고 `pg`가 주는 `constraint` 필드가 없다. 표준 CHECK·UNIQUE 위반은 이름이 `message`에 문자열로 들어오지만, plpgsql `RAISE … using constraint = …`(I-16과 쓰기 함수)는 메시지가 한글 커스텀 문구다. **P3b 2단계에서 네 형태를 실제로 위반시켜 본문을 측정하고 그 결과를 이 자리에 기입한다** — 측정 전에는 파싱 대상을 정하지 않는다.
...
|---|---|---|
| `dDay` · `isPast` (V-07 검증) | 날짜 형식 · 존재하지 않는 날짜 | `VALIDATION_FAILED` + 해당 날짜 필드 |
...
| W-04 | 금액·비율은 **문자열로 보내고** DB 경계에서 `::numeric`으로 캐스팅한다 | Q-08의 쓰기 방향이다. 숫자로 실으면 JS 쪽에서 이미 float64를 경유했으므로 DB가 받는 값이 무엇이든 늦다. 쓰기 함수의 `jsonb` payload도 같다 — `->>`로 텍스트를 꺼내 `::numeric`으로 캐스팅하며 `->`로 JSON 수치를 쓰지 않는다 |
...
| SCR-302 시세 | `listAssetPrices` | `saveManualPrice`, `refreshPrices` (**공급자 미선정 — §5.8**) |
