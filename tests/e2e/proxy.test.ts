...
describe('미인증 게이트', () => {
  ...
  it('api/cron은 매처 밖이다 — Cron이 /login으로 튕기지 않는다', async () => {
     ...
     * 죽는 종류라 지금 고정한다.
     *
     * **후자가 404 → 401로 바뀌었다 (P5b 컷 1).** 종전 주석은 「라우트는 아직 없으므로
     * (P5) 404가 정답이며, 307이면 매처 결함이다」로 **조건을 함께** 적어 두었고, 그 조건이
     * 변한 것이다 — 라우트가 섰으므로 미인증 호출의 정답은 CR-01의 401이다. 음성 대조
     * (`not.toBe(307)`)는 그대로 살아 있고, **404가 아님**은 「라우트가 실재한다」를 함께
     * 고정한다(둘 중 하나만 쓰면 다른 하나가 통과한다).
     *
     * 응답의 내용(본문·헤더·토큰 갈래)은 `tests/e2e/cron.test.ts`가 본다. 여기 남는 것은
     * **매처**에 대한 진술 하나다.
     */
