 ...
 *
 * ## `aria-current="page"`는 주 네비게이션의 것이다 (P4 컷 9)
 *
 * 이 함수는 **문서 전체**를 훑으므로 다른 컴포넌트가 같은 표식을 쓰면 아래 개수
 * 단언이 잘못된 이유로 깨진다 — 실제로 컷 9에서 걸렸다(SCR-101의 범위 전환이
 * `"page"`를 쓰자 홈의 활성 표식이 3건이 되었다). 그쪽은 같은 화면의 다른
 * **상태**이지 다른 페이지가 아니므로 `aria-current="true"`로 고쳤다.
 *
 * 즉 이 단언들이 그 규약의 게이트다: 「페이지 단위 활성 표식은 주 네비게이션만
 * 쓴다」를 어기면 여기가 빨간불이 된다. 문서로 적는 대신 개수로 고정한다.
 */
