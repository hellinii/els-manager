...
describe('cron_runs — 조회는 공용, 쓰기는 INSERT 뿐', () => {
  ...
  it('★ 남의 id로는 남길 수 없다 — actor_id가 위조 불가다', async () => {
    /*
     * 이것이 DOC-013 §7.2 근거 ②(호출자 판별)를 «닫는» 성질이다. `caller`는 요청이
     * 스스로 주장하는 값이라 판별자가 아니고, 위조 불가한 것은 이 열뿐이다.
     *
     * ★ 그리고 이 성질은 AQ-57의 답에 «의존한다» — `service_role`은
     * `rolbypassrls = t`라서 이 정책이 아예 적용되지 않는다. ⓐ(전용 서비스 계정)를
     * 택했기 때문에 이 케이스가 뜻을 갖는다.
     */
