 ...
 *
 * ~~**수집 배치가 없는 동안 필요 권한은 정말로 0이다** — 배치가 생기는 날(P5a 컷 4)
 * 필요한 셀만 여기서 채운다.~~
 *
 * **★ 배치가 왔고(P5a 컷 3) 채워진 셀은 «0»이었다** — 컷 4가 그것을 확정했다.
 * 배치는 `service_role`이 아니라 GoTrue 계정으로 로그인해 `authenticated`로 돌고,
 * 그 롤이 필요한 넷을 이미 갖고 있었다(`els_products`·`assets`·
 * `asset_provider_symbols` SELECT + `asset_prices` INSERT). 새로 부여된 것은
 * `cron_runs` INSERT 하나이며 컷 2a가 테이블과 함께 부여했다.
 * **즉 이 행들은 «아직 안 채운 것»이 아니라 «채울 것이 없다는 답»이다** (AQ-11 종결).
 *
...
export const TABLE_PRIVILEGES = {
   ...
   * ⓐ `service_role`이 0 DML이고 AQ-57이 ⓐ(GoTrue 사용자 계정)로 닫혔으므로 배치는
