 ...
 * `GET /` → 200. `GET /api/cron/prices` → **401**(토큰 없음, DOC-011 §7.1 CR-01)이지
 * 307이 아니다 — 매처 예외가 실제로 동작한다.
 *
 * **컷 0b의 이 값은 404였다**(라우트가 없었다). P5b 컷 1이 Route Handler를 세우면서
 * 401로 바뀌었고, 그때 이 인용을 함께 고쳤다 — **실측 기록이 낡으면 그것은 거짓 인용이고
 * 이 저장소는 그것을 결함으로 다룬다.** 매처에 대한 진술은 변하지 않았다(307이 아니다).
 *
