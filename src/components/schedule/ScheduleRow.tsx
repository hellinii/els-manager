 ...
 *
 * ## `deriveDisplay()`의 **다섯 번째 소비자**가 되었다
 *
 * 그 함수의 머리글이 「이 판정을 하는 화면이 다섯」이라고 적었는데 v1.7까지 이
 * 화면은 그중 하나가 될 수 없었다 — 뷰에 `status`가 없어 §4.2 우선순위표의 E-05
 * 단을 판정할 입력이 모자랐다(DOC-011 §4.4 v1.8이 신설한 이유). 그래서 이 파일은
 * 우선순위를 다시 정하지 않고 `kind`만 분기한다.
 *
 ...
 * §4.2 우선순위표의 네 갈래 + 판정.
 *
 * **판정은 적용 차수에만 붙는다**(§4.4 — `conditionResult`는 `next`와 같은 차수에서만
 * 값이 있다). 그러므로 이 칸이 비어 있는 것은 결함이 아니라 「그 차수는 지금 판정
 * 대상이 아니다」이며, 우선순위 넷과는 다른 이유다 — 넷은 **왜 판정할 수 없는가**를
 * 말하고 이쪽은 **판정할 차수가 아니다**를 말한다.
 */
