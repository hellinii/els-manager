 ...
 *
 * 방어적 선택이며 근거는 추정이다 — 보이는 후보마다 상세 조회(키움 요청 넷)가 날 수 있다는 것을
...
export function KiwoomImport({
  ...
  return (
    ...
    >
      ...
      <form method="GET" action={PATHS.productNew} className="flex flex-wrap items-end gap-2">
        ...
        <p className="w-full text-xs text-neutral-500">
          상품명의 번호를 적는다(차수가 아니다). 불러오면 아래 폼이 새로 채워진다 — 투자원금·계좌유형은
          불러온 뒤 적는다.
        </p>
      </form>

      {state.message != null && <Notice tone={state.tone}>{state.message}</Notice>}
      ...
      {panel.code != null && (
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          {panel.query != null && (
            <Link
              href={importHref({ query: panel.query, code: null })}
              prefetch={false}
          ...
          <Link href={PATHS.productNew} prefetch={false} className="text-neutral-600 underline">
            불러오기 취소
          </Link>
        </div>
      )}
        ...
        <ul className="flex flex-col divide-y divide-neutral-100 rounded-md border border-neutral-200">
          {panel.candidates.map(({ candidate, exact, refusal }) => (
            <li key={candidate.productCode} className="flex flex-col gap-1 px-3 py-2 text-sm">
              ...
              {refusal == null ? (
                <Link
                  href={importHref({ query: panel.query, code: candidate.productCode })}
                  prefetch={false}
