 ...
 *
 * **이 산식은 정본이 아니라 빈 평가일 칸의 기본값이다 (DOC-002 §4.8 v1.6).** 키움 공개
 * 상품 페이지에서 상품마다 다른 규약이 실측되어(E04000: 0일 + 다음 영업일) 평가일이
 * 차수별 입력값이 됐다. 이 파일의 계산은 바뀌지 않았다 — 부르는 쪽이 「빈 칸을 채울 때」로
 * 좁혀졌을 뿐이다(`lib/forms/schedules.ts`의 `applyEvaluationDates`).
 */
 ...
 *
 * **전역 규약이므로 상품별 차이를 표현하지 못한다** — 그리고 실제로 그런 상품이 나왔다
 * (RD-03 ⓑ, DOC-007 v1.1). 그래서 평가일이 차수별 입력값이 되었고 이 상수는 **빈 칸의
 * 기본값**을 정한다. 다른 규약의 상품은 칸에 실제 날짜를 적고(또는 불러오고), 칸 옆 힌트가
 * 산식과의 차이를 보여 준다.
 */
