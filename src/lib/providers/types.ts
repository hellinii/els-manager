 ...
 * 등록된 공급자 — **아직 비어 있다.**
 *
 * ## 어댑터가 «있는데» 등재하지 않는 이유 (P5a 컷 1)
 *
 * `providers/kiwoom/`에 실제로 동작하는 어댑터가 섰다. 그런데 여기 넣으면
 * `providerCount > 0`이 되고 **수집기가 없으므로** DOC-011 §5.8이 CR-08(`INTERNAL` + 로깅)로
 * 끝난다 — 그것은 「배포 결함」 상태이며 **503(설계된 상태)보다 나쁘다.**
 * R-05(각 단계는 동작하는 상태에서 끝낸다)를 지키려면 **등재는 수집기와 같은 컷**이어야
 * 하고 그것이 P5a 컷 3이다.
 *
 ...
 * `ok: true` + `succeeded: 0`이 **아니다** — 전자는 ST-03(수동 입력 폴백)으로 유도하고
